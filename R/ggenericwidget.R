...
setMethod(".ggenericwidget",
          ...
          function(toolkit,
            ...
            if(!is.null(lst$assignto)) {
              frame = gframe("<b>Assign output to:</b>", markup=TRUE, container=mainGroup)
...
#                  container=gwindow(visible=TRUE))
            ...
            obj = new("gGenericWidgetRGtk",block=mainGroup, widget=mainGroup,
              toolkit=toolkit, lst = lst)
            invisible(obj)
})
...
## return the list
setMethod(".svalue",
          signature(toolkit="guiWidgetsToolkitRGtk2",obj="gGenericWidgetRGtk"),
